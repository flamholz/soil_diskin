...
from soil_diskin.models import ReducedComplexModel
...
for model in ['CESM','IPSL','MRI']:
    ...
    all_site_params.append(xr.concat([extrapolated.sel(lat=site_data.iloc[i]['Latitude'],lon=site_data.iloc[i]['Longitude'],method='ffill') for i in range(site_data.shape[0])],dim='site'))

all_site_params = xr.concat(all_site_params,dim='model')
all_site_params['model'] = ['CESM','IPSL','MRI']
...
def RCM_predict_site(model_name, i, RCM_config, RCM_params, site_data):
    ...
    RCM_config.model = model_name
    RCM_params.params = all_site_params.sel(model=[model_name])[0, i, :].values
...
RCM_predictions.to_csv(f'results/04_model_predictions/RCM.csv', index=False)
# %%
