...
params = CSV.read("results/03_calibrate_models/03b_lognormal_predictions_calcurve_julia.csv", DataFrame); 
...
for i in ProgressBar(1:size(params, 1))
    ...
    _, result = run_diskin_fast(params[i,:turnover], params[i,:pred], 1.0, tmax=tmax, ts_size=ts_size);
...
for i in ProgressBar(1:size(backfilled_params, 1))
        # run the diskin model for each row in params
    _, result = run_diskin_fast(backfilled_params[i,:turnover_q05], backfilled_params[i,:pred_05], 1.0, tmax=tmax, ts_size=ts_size);
    _, result2 = run_diskin_fast(backfilled_params[i,:turnover_q95], backfilled_params[i,:pred_95], 1.0, tmax=tmax, ts_size=ts_size);
...
CSV.write("results/04_model_predictions/04b_lognormal_cdfs_julia.csv",  Tables.table(results, header = ts));
CSV.write("results/04_model_predictions/04b_lognormal_cdfs_05_julia.csv",  Tables.table(results_05, header = ts));
CSV.write("results/04_model_predictions/04b_lognormal_cdfs_95_julia.csv",  Tables.table(results_95, header = ts));
