...
raw_site_data = CSV.read("results/raw_balesdant_2018.csv" ,DataFrame); 
ratios = raw_site_data[:,"Cref_0-100estim"] ./ raw_site_data[:, "Ctotal_0-100estim"];

mean_age = CSV.read("results/03b_lognormal_site_parameters.csv", DataFrame, header=false);
mean_age = mean(mean_age[:,1]);

mean_turnover = CSV.read("results/tropical_sites_14C_turnover.csv", DataFrame);
mean_turnover = mean(mean_turnover[:,"turnover"]);
