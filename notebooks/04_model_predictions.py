...
np.savetxt(f'results/04_model_predictions/gamma_{pd.Timestamp.now().date().strftime("%d-%m-%Y")}.csv', predictions)
...
predictions = np.array(predictions)
# Save the model predictions
np.savetxt(f'results/04_model_predictions/power_law_{pd.Timestamp.now().date().strftime("%d-%m-%Y")}.csv', predictions)
...
predictions = np.array(predictions)
# Save the model predictions
np.savetxt(f'results/04_model_predictions/general_power_law_{pd.Timestamp.now().date().strftime("%d-%m-%Y")}.csv', predictions)
