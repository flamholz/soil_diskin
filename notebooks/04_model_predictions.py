...
# TODO: there is an asymmetry between the way we handle the continuum 
# and compartmental models. For the lognormal, gamma, powerlaw etc. we
# do the heavy lifting in prior scripts and generate output here.
# For the compartmental models we do everything here. This means that
# we need to rerun this script even if something minor fails... 
...
# Save the model predictions
predictions.to_csv(f'results/04_model_predictions/CLM45_{current_date}.csv', index=False)
np.savetxt(f'results/04_model_predictions/CLM45_fnew_{current_date}.csv', fnew_predictions)
...
# Save the model predictions
JSBACH_predictions.to_csv(f'results/04_model_predictions/JSBACH_{current_date}.csv', index=False)
np.savetxt(f'results/04_model_predictions/JSBACH_fnew_{current_date}.csv', JSBACH_fnew_predictions)
