...
#This script calculates mean ages and transit times as well as quantiles for output from the IPSL model with corrected values from He's paper
...
for(i in 1:nrow(ipsl)){
  A=diag(-1/ipsl[i,2:4])
  A[3,3]=-1/(ipsl[i,4]*14) #corrected
  A[2,1]=ipsl[i,5]/ipsl[i,2]
  A[3,2]=0.07*ipsl[i,6]/ipsl[i,3] #corrected
...
write.csv(outdf,"~/SOIL-R/Manuscripts/Persistence/CodeData/He/IPSL/corrIPSLages.csv", row.names = FALSE )
